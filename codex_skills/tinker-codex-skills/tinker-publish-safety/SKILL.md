...
- Public or semi-public code/docs/dashboard/skills: publish through the main Tinker repo and later Public Maintainer when appropriate.
- Private data/storage: keep under `private-user`, usually as a nested repo inside `data/`.
...
3. Confirm `data/training_data/`, run outputs, logs, checkpoints, env files, and local venvs are ignored.
...
git -C data\training_data commit -m "Update private training data training data"
...
- `public maintainer`: ML-related, semi-professional public-facing blog, pseudo-startup, and computer-led research work.
- `private-user`: personal projects and private/storage repos.

Default to `private-user` for raw training data and local storage. Move polished public research/code to Public Maintainer only after explicit user direction.
