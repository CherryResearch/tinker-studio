...
- Dataset panels default to `data\training_data_cerise`, which is ignored by the main Tinker repo and may be a private nested repo.
...
- Dataset tab: `data\training_data_cerise\processed\posts.csv`, reply-context columns, and `tinker\dataset_manifest.json`.
...
- Sidebar refresh: calls `data\training_data_cerise\build_bluesky_finetune_dataset.py --handle <handle> --outdir <dataset-root>`.
